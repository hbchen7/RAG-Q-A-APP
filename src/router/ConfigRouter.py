...
async def LLMConfigSave(user_LLMConfig: LLMConfigIn):
  ...
  userLLMConfig = UserLLMConfig(username=user_LLMConfig.username, supplier=user_LLMConfig.supplier, model=user_LLMConfig.model, apiKey=user_LLMConfig.apiKey)
...
async def EnbeddingConfigSave(user_EnbeddingConfig: EnbeddingConfig):
  # 如果已存在该用户的模型配置，则更新该配置，否则创建新的配置
  existing_config = await UserEnbeddingConfig.find_one(UserEnbeddingConfig.username==user_EnbeddingConfig.username,UserEnbeddingConfig.model == user_EnbeddingConfig.model)
  if existing_config:
    existing_config.supplier = user_EnbeddingConfig.supplier
    existing_config.apiKey = user_EnbeddingConfig.apiKey
    await existing_config.save()
    return existing_config
  userEnbeddingConfig = UserEnbeddingConfig(username=user_EnbeddingConfig.username, supplier=user_EnbeddingConfig.supplier, model=user_EnbeddingConfig.model, apiKey=user_EnbeddingConfig.apiKey)
...
async def LLMConfigGet(current_user = Depends(get_current_user)):
  print(current_user)
  userLLMConfig = await UserLLMConfig.find(UserLLMConfig.username == current_user.username).to_list()
  return userLLMConfig
