...
class ChatSev:
    ...
    def __init__(self, knowledge: Optional[Knowledge], chat_history_max_length:Optional[int] = 8):

        self.knowledge: Optional[Knowledge] = knowledge
        self.chat_history_max_length: int = chat_history_max_length if chat_history_max_length is not None else 8
    ...
    def get_chain(self, collection: str, supplier:str,model: str, max_length: int, temperature: float=0.8) -> RunnableWithMessageHistory:
        """获取聊天链"""
        # chat = ChatOpenAI(model=model, max_tokens=max_length, temperature=temperature)
        chat = get_llms(supplier=supplier,model=model, max_length=max_length, temperature=temperature)
    ...
    def invoke(self, question: str, collection: Optional[str] ,supplier:str, model: str,
               max_length=None, temperature=0.8) -> Output:
        """
        ...
        return self.get_chain(
            collection,supplier, model, max_length, temperature
        ).invoke(
            {"input": question}
        )
